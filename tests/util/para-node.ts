...
export async function startAllychainNodes(options: ParaTestOptions): Promise<{
  ...
  const allyChain = options.allychain.chain || "moonbase-local";
  const paraBinary =
  ...
  const paraSpecs =
    "spec" in options.allychain
      ...
      ? await generateRawSpecs(paraBinary, allyChain)
      : await getRawSpecsFromTag(allyChain.split("-")[0] as any, options.allychain.runtime);
